...
@api.get(
    ...
    description=("Retrieve a list of all devices or detailed information about specific devices."),
)
...
@api.put(
    ...
    description=(
        "Associate or update multiple devices linked to beds in a single operation. "
        "Ideal for batch assigning or reconfiguring devices in bulk."
    ),
)
...
@api.get(
    ...
    response_class=FastJSONResponse,
    description=(
        "Retrieve the operational range or configuration details of a "
        "specific device identified by its unique ID."
    ),
)
