...
from src.auth.schemas import LoginCredentials, LogoutSchema, WebChangePasswordPayload
...
if settings.AUTH_V2_ENABLED:
    ...
    @api.post(
        ...
        operation_id="post-web-auth-session-api",
        ...
        description=(
            ...
            "returns an a session cookie."
        ),
    )
    async def open_new_session(response: Response, payload: LoginCredentials) -> Response:
        token = await auth_client.get_internal_token_v2(payload)
        response.set_cookie(
            "sibel-sc-session",
            value=token,
            httponly=True,
            samesite="strict",
            domain=settings.APP_DOMAIN_NAME,
...
async def get_internal_token_api(
    payload: auth_schemas.LoginCredentials,
) -> FastJSONResponse:
...
async def get_technical_internal_token_api(
    payload: auth_schemas.TechnicalLoginCredentials,
) -> FastJSONResponse:
    token = await auth_client.get_internal_token(payload)
    return FastJSONResponse(token)
