...
async def get_internal_token(
    payload: LoginCredentials | TechnicalLoginCredentials,
) -> InternalToken:
...
async def get_internal_token_v2(
    payload: LoginCredentials | TechnicalLoginCredentials,
) -> InternalToken:
    url = f"{settings.AUTH_PLATFORM_V2_BASE_URL}/Login"
    async with httpx.AsyncClient() as client:
        headers = {
            "Content-Type": "application/json",
        }
