...
from src.cache import clear_monitor_keep_alive, update_monitor_keep_alive
...
async def process_consumer_message(message_topic: str, message: dict) -> None:
    if message_topic == settings.PM_STATE_TOPIC:
        ...
        if event_type == "PM_CONNECTION_STATUS_REPORT":
            monitor_id = message["payload"]["device_primary_identifier"]
            update_monitor_keep_alive(monitor_id)
        elif event_type == "DISCONNECT":
            monitor_id = message["pm_id"]
            clear_monitor_keep_alive(monitor_id)
