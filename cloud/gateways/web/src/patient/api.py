...
@api.post(
    ...
    description=(
        "Onboarding new patients into the system with their initial details. "
        "This endpoint is not usually needed since patients are added by starting"
...
@api.delete(
    ...
    description="Remove a specific patient record.",
)
...
@api.get(
    ...
    description=("Retrieve alerts associated with a specific patient session."),
)
...
@api.get(
    ...
    response_model_by_alias=True,
    description=(
        "Retrieve patient information from the integrated EHR. "
        "Requires EHR integration enabled to be enabled."
    ),
)
...
@api.post(
    ...
    response_model_by_alias=True,
    description=(
        "Admit a patient into the system. There are two possible operation flows,"
        "`quick admit` and `ehr admit`. Choosing `quick admit` means that the information"
        "passed to the system will be used to start a patient monitoring session."
        " On the other side `ehr admit` will search for patient information in the integrated"
        " EHR and upon confirmation pass that information along to the patient monitor. "
        "If the admission succeeds this only means that the admission was requested, as still "
        "needs to be confirmed from bed side."
    ),
)
...
@api.delete(
    ...
    description=(
        "In case of rejection of the patient admission from bed side, this API allows "
        "removing the admission request."
    ),
)
