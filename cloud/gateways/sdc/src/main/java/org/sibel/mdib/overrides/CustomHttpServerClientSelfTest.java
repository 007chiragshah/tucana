 ...
 */
public class CustomHttpServerClientSelfTest {
    private static final Logger LOG = LogManager.getLogger(CustomHttpServerClientSelfTest.class);
    ...
    private final CustomJettyHttpServerHandlerFactory jettyHttpServerHandlerFactory;
    private final CryptoConfigurator cryptoConfigurator;
    private final CryptoSettings cryptoSettings;
    private SSLContext sslContext;

    @Inject
    public CustomHttpServerClientSelfTest(
    ...
    private SSLContext getSslContext() {
        try {
            ...
            if (cryptoSettings.getKeyStoreStream().isEmpty()
                    || cryptoSettings.getTrustStoreStream().isEmpty()) {
            ...
            sslContextBuilder.loadKeyMaterial(
                    keyStore, cryptoSettings.getKeyStorePassword().toCharArray());
    ...
    private HttpClientBuilder buildHttpClient() {
        return HttpClients.custom()
                ...
                .setSSLSocketFactory(
                        new SSLConnectionSocketFactory(sslContext, tlsProtocols, enabledCiphers, hostnameVerifier));
    }
    ...
    private ServerConnector getServerConnector(URI uri, Server server) {
        ...
        ServerConnector httpsConnector =
                new ServerConnector(server, getContextFactory(), new HttpConnectionFactory(httpsConfig));
    ...
    private ContextHandler getHttpHandler(String contextPath) {
        var handler = new HttpHandler() {
            @Override
            public void handle(
                    InputStream inStream, OutputStream outStream, CommunicationContext communicationContext) {
                try {
        ...
        CustomJettyHttpServerHandler endpointHandler =
                this.jettyHttpServerHandlerFactory.create(SoapConstants.MEDIA_TYPE_SOAP, handler);
