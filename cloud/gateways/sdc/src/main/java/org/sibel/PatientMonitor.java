...
import org.sibel.models.events.DisconnectEvent;
...
public class PatientMonitor {
    ...
    private WatchdogObserver watchdogObserver;
    ...
    public PatientMonitor(
            ...
            @Assisted DisconnectListener disconnectListener,
    ...
    public void connect() throws FailedToConnectPatientMonitor {
        try {
            ...
            watchdogObserver = new WatchdogObserver() {
                @Subscribe
    ...
    public void notifyDiscovery() throws ActionOnDisconnectedPatientMonitor {
        ...
        List<MetricPayload<?>> initialMetricValues = !patientAlreadyInUse && patient != null
                ? getInitialMetricValues(patient.getPrimaryIdentifier())
                : List.of();
        var deviceDiscoveredMessage = new DeviceDiscoveredBrokerMessage(
                uuidProvider.get(),
                instantProvider.now(),
                new DeviceDiscoveredPayload(gateway, patientAlreadyInUse ? null : patient, initialMetricValues));
    ...
    private void handleOngoingPmEncounters(PatientPayload patient) {
    ...
    private void notifyDisconnection() {
        var pmId = serialNumber != null
                ? "id:%s".formatted(serialNumber)
                : "epr:%s".formatted(discoveredDevice.getEprAddress());

        LOG.info("Executing watchdog for PM {}.", pmId);
        try {
            disconnectListener.onDisconnect();
        } catch (Exception e) {
            LOG.warn("Failed to run disconnect listener for PM {}.", pmId, e);
        }
        try {
            if (serialNumber != null) {
                kafkaProducer.notify(
                        new DisconnectEvent(uuidProvider.get(), instantProvider.now(), serialNumber, getPatientId()),
                        settings.KAFKA_TOPIC_SDC_REALTIME_STATE(),
                        serialNumber,
                        List.of());
            }
        } catch (Exception e) {
            LOG.warn("Failed to notify device disconnection event for PM {}.", pmId, e);
        }
    }

    private List<MetricPayload<?>> getInitialMetricValues(String patientPrimaryIdentifier)
            throws ActionOnDisconnectedPatientMonitor {
        var initialMetrics = new ArrayList<MetricPayload<?>>();
        var mdibAccess = getMdibAccess();

        if (patientPrimaryIdentifier != null && serialNumber != null) {
            var metricStates = mdibAccess.getStatesByType(AbstractMetricState.class).stream()
                    .filter(state -> !(state instanceof RealTimeSampleArrayMetricState))
                    .toList();

            for (var metricState : metricStates) {
                try {
                    var processor = processorFactory.createMetricChangeProcessor(
                            mdibAccess, metricState, patientPrimaryIdentifier);
                    initialMetrics.add(processor.getMetricPayload());
                } catch (Exception e) {
                    LOG.warn("Failed to load initial metric state", e);
                }
            }

            var componentStates = mdibAccess.getStatesByType(BatteryState.class);
            for (var componentState : componentStates) {
                try {
                    var processor = processorFactory.createComponentStateChangeProcessor(
                            mdibAccess, componentState, serialNumber, patientPrimaryIdentifier);
                    initialMetrics.add(processor.getBatteryMetricPayload());
                    initialMetrics.add(processor.getChargingStatusMetricPayload());
                } catch (Exception e) {
                    LOG.warn("Failed to load initial metric state", e);
                }
            }
