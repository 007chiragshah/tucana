...
public class MdibAccessBuilder {
    ...
    public MdibAccessBuilder addAlert(SensorType sensorType, TechnicalAlertSignal alert, boolean enabled) {
        return addAlert(sensorType, alert, enabled, null);
    }

    public MdibAccessBuilder addAlert(
            SensorType sensorType, TechnicalAlertSignal alert, boolean enabled, Instant determinationTime) {
    ...
    public MdibAccessBuilder addAlert(
            ...
            Instant determinationTime,
            boolean withSources) {
        var alertConditionDescriptor = createAlertConditionDescriptor(sensorType, alert.condition, withSources);
        if (!modification.isAddedAsInserted(alertConditionDescriptor.getHandle())) {
            modification.insert(
                    alertConditionDescriptor,
                    createAlertConditionState(
                            sensorType, alert.condition, determinationTime, enabled, AlertActivation.ON),
    ...
    public MdibAccessBuilder addAlert(PhysiologicalAlertSignal alert, AlertSignalPresence presence) {
        return addAlert(alert, presence, null);
    }

    public MdibAccessBuilder addAlert(
            PhysiologicalAlertSignal alert, AlertSignalPresence presence, Instant determinationTime) {
        var alertConditionDescriptor = createAlertConditionDescriptor(alert.condition);
        if (!modification.isAddedAsInserted(alertConditionDescriptor.getHandle())) {
            modification.insert(
                    alertConditionDescriptor,
                    createAlertConditionState(
                            alert.condition, determinationTime, presence == AlertSignalPresence.ON, AlertActivation.ON),
                    getAlertSystemHandle(SensorType.PM));
        }
        modification.insert(
                createAlertSignalDescriptor(alert),
                createAlertState(alert, presence),
                getAlertSystemHandle(SensorType.PM));
        return this;
    }
