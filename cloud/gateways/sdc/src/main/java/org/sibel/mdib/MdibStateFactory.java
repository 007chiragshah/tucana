...
public class MdibStateFactory {
    ...
    public static LimitAlertConditionState createAlertConditionState(
        ...
        state.setActivationState(activation);
        state.setMonitoredAlertLimits(AlertConditionMonitoredLimits.ALL);
        state.setLimits(createLimits(alert.upperLimit, alert.lowerLimit));
        state.setPresence(presence);
        state.setDeterminationTime(determinationTime);
        return state;
    }

    public static LimitAlertConditionState createAlertConditionState(PhysiologicalAlertCondition alert) {
        return createAlertConditionState(alert, null, false, AlertActivation.ON);
    }

    public static LimitAlertConditionState createAlertConditionState(
            PhysiologicalAlertCondition alert, Instant determinationTime, boolean enabled, AlertActivation activation) {
        var state = new LimitAlertConditionState();
    ...
    public static AlertSignalState createAlertState(PhysiologicalAlertSignal alert, AlertSignalPresence presence) {
        var state = new AlertSignalState();
