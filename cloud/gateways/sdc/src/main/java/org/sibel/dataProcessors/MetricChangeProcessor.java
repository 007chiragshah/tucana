...
public class MetricChangeProcessor {
    ...
    public BrokerMessage getBrokerMessage() throws ProcessingError {
        try {
            return new MetricBrokerMessage<>(uuidProvider.get(), instantProvider.now(), getMetricPayload());
    ...
    public MetricPayload<?> getMetricPayload() throws ProcessingError {
        MetricPayload<?> payload;
        if (state instanceof NumericMetricState) {
            payload = getNumericMetricPayload();
        } else if (state instanceof StringMetricState) {
            payload = getStringMetricPayload();
        } else {
