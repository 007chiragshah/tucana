...
class TestConsumerReportProcessor extends BaseIntegrationTest {
    ...
    */
    ...
    void testTechnicalAlertChangeReceived(boolean withSources, boolean withDeterminationTime)
        ...
        var devicePrimaryIdentifier = ANNE_CHEST_ID;
        var alert = TechnicalAlertSignal.LEAD_OFF;
        ...
        var expectedMessage = new AlertBrokerMessage(
                ...
                new AlertPayload(
                        ...
                        alert.code,
                        deviceType.code,
                        alert.condition.priority.toString(),
    ...
    void testTechnicalAlertChangeReceivedWithoutPatientData() throws PreprocessingException {
        connectDefaultPatientMonitor(null); // No patient
        var modificationMessage = createAlertChangeModificationMessage(
                SensorType.ANNE_CHEST, ANNE_CHEST_ID, TechnicalAlertSignal.LEAD_OFF, true, true, true);
        processor.onAlertChange(modificationMessage);
        kafkaProducerMock.verifyNoMessagesSent();
    }
    ...
    void testPhysiologicalAlertChangeReceived() throws PreprocessingException {
        var modificationMessage = createAlertChangeModificationMessage(
                SensorType.ANNE_CHEST, ANNE_CHEST_ID, PhysiologicalAlertSignal.RR_HIGH__VIS, true);
        processor.onAlertChange(modificationMessage);
        ...
        var expectedMessage = new AlertBrokerMessage(
                ...
                new AlertPayload(
                        ...
                        true,
                        false,
                        PhysiologicalAlertSignal.RR_HIGH__VIS.code,
                        SensorType.ANNE_CHEST.code,
                        PhysiologicalAlertSignal.RR_HIGH__VIS.condition.priority.toString(),
                        ALERT_DETERMINATION_TIME,
                        ANNE_CHEST_ID,
                        new VitalsRangePayload(
                                PhysiologicalAlertSignal.RR_HIGH__VIS.condition.code,
                                PhysiologicalAlertSignal.RR_HIGH__VIS.condition.upperLimit,
                                PhysiologicalAlertSignal.RR_HIGH__VIS.condition.lowerLimit,
                                true)));
        kafkaProducerMock.verifyMessageSent(
    ...
    void testPhysiologicalAlertWithLatchingReceivedWhereConditionIsFalse() throws PreprocessingException {
        var modificationMessage = createAlertChangeModificationMessage(
                SensorType.ANNE_CHEST, ANNE_CHEST_ID, PhysiologicalAlertSignal.RR_HIGH__VIS, false, true);
        processor.onAlertChange(modificationMessage);
    ...
    void testLimitAlertChangeReceived() throws PreprocessingException {
        var alerts = List.of(PhysiologicalAlertSignal.RR_HIGH__VIS, PhysiologicalAlertSignal.HR_HIGH__VIS);
    ...
    void testSensorChangeReceived() throws PreprocessingException {
        ...
        var expectedMessage = new DeviceDiscoveredBrokerMessage(
                TestConstants.UUID_LIST.get(0),
                TestConstants.NOW,
                new DeviceDiscoveredPayload(devicePayload, TestConstants.CONNECTED_PATIENT, List.of()));
        kafkaProducerMock.verifyMessageSent(
                settings.KAFKA_TOPIC_DEVICE(), TestConstants.CONNECTED_PM_ID, expectedMessage, null);
    }
    ...
    private AlertStateModificationMessage createAlertChangeModificationMessage(
        ...
        var mdibAccess = new MdibAccessBuilder()
                ...
                .addMetric(sensorType, alert.condition.metric, BigDecimal.ZERO, METRIC_DETERMINATION_TIME)
        ...
        setConnectedPmMdibAccess(mdibAccess);
        var alertState =
                createAlertState(sensorType, alert, enabled ? AlertSignalPresence.ON : AlertSignalPresence.OFF);
        return new AlertStateModificationMessage(mdibAccess, Map.of("key", List.of(alertState)));
    }

    private AlertStateModificationMessage createAlertChangeModificationMessage(
            SensorType sensorType, String devicePrimaryIdentifier, PhysiologicalAlertSignal alert, boolean enabled)
            throws PreprocessingException {
        var mdibAccess = createMdibAccessWithPhysiologicalAlerts(
                sensorType, devicePrimaryIdentifier, List.of(alert), enabled, false);
        setConnectedPmMdibAccess(mdibAccess);
        var alertState = createAlertState(alert, enabled ? AlertSignalPresence.ON : AlertSignalPresence.OFF);
        return new AlertStateModificationMessage(mdibAccess, Map.of("key", List.of(alertState)));
    }
    ...
    private AlertStateModificationMessage createAlertChangeModificationMessage(
            SensorType sensorType,
            String devicePrimaryIdentifier,
            PhysiologicalAlertSignal alert,
            boolean enabled,
            boolean latching)
            throws PreprocessingException {
        var mdibAccess = createMdibAccessWithPhysiologicalAlerts(
        ...
        var alertState = createAlertState(alert, presence);
        return new AlertStateModificationMessage(mdibAccess, Map.of("key", List.of(alertState)));
    }

    private AlertStateModificationMessage createLimitAlertChangeModificationMessage(
            SensorType sensorType, String devicePrimaryIdentifier, List<PhysiologicalAlertSignal> alerts)
            throws PreprocessingException {
        var mdibAccess =
    ...
    @SuppressWarnings("OptionalGetWithoutIsPresent")
    private DescriptionModificationMessage createLimitDescriptionModificationMessage(
            List<PhysiologicalAlertSignal> alerts) throws PreprocessingException {
        var mdibAccess =
    ...
    private static MdibAccess createMdibAccessWithPhysiologicalAlerts(
            SensorType sensorType,
            String devicePrimaryIdentifier,
            List<PhysiologicalAlertSignal> alerts,
            boolean enabled,
            boolean latching)
            throws PreprocessingException {
        var mdibAccessBuilder = new MdibAccessBuilder()
        ...
        for (var alert : alerts) {
            var presence =
                    latching ? AlertSignalPresence.LATCH : enabled ? AlertSignalPresence.ON : AlertSignalPresence.OFF;
            mdibAccessBuilder.addAlert(alert, presence, ALERT_DETERMINATION_TIME);
