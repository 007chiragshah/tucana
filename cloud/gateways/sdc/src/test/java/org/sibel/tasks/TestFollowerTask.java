...
public class TestFollowerTask extends BaseIntegrationTest {
    ...
    private void verifyPmReleased() {
        verify(redisMock, atLeast(1)).del(RedisKeys.getDeviceKey(TestConstants.CONNECTED_PM_EPR));
        verify(redisMock, atLeast(1))
    ...
    private void verifyDeviceDiscoveredNotificationSent(
        ...
        List<MetricPayload<?>> initialMetricValues = patientPayload != null
                ? List.of(
                        new MetricPayload<>(
                                "P-001",
        ...
        var deviceDiscoveredMessage = new DeviceDiscoveredBrokerMessage(
                ...
                TestConstants.NOW,
                new DeviceDiscoveredPayload(devicePayload, patientPayload, initialMetricValues));
    ...
    private void verifyNewVitalRangesSent() {
        var rrAlertCondition = PhysiologicalAlertSignal.RR_HIGH__VIS.condition;
        var leadOffAlertCondition = TechnicalAlertSignal.LEAD_OFF.condition;
    ...
    private LocalMdibAccess createMdibAccess(PatientPayload patient, AlertActivation audioActivation)
        ...
        var builder = new MdibAccessBuilder()
                ...
                .addMetric(SensorType.ANNE_CHEST, Metric.DEVICE_LEAD, "true", TestConstants.METRIC_DETERMINATION_TIME)
                .addMetric(
                ...
                .addAlert(SensorType.ANNE_CHEST, TechnicalAlertSignal.LEAD_OFF, true, ALERT_DETERMINATION_TIME)
                .addAlert(PhysiologicalAlertSignal.RR_HIGH__VIS, AlertSignalPresence.OFF, ALERT_DETERMINATION_TIME)
    ...
    private static List<AlertPayload> createTechnicalAlerts(PatientPayload patientPayload) {
        var leadOffAlertPayload = new AlertPayload(
                ...
                true,
                false,
                TechnicalAlertSignal.LEAD_OFF.code,
                SensorType.ANNE_CHEST.code,
                TechnicalAlertSignal.LEAD_OFF.condition.priority.toString(),
                ALERT_DETERMINATION_TIME,
                ANNE_CHEST_ID,
                new VitalsRangePayload(
                        TechnicalAlertSignal.LEAD_OFF.condition.code,
                        TechnicalAlertSignal.LEAD_OFF.condition.upperLimit,
                        TechnicalAlertSignal.LEAD_OFF.condition.lowerLimit,
                        true));
