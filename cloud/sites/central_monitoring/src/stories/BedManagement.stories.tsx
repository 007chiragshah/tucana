...
export const MoveBetweenSteps: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(async () => {
      ...
      await expect(canvas.queryByTestId('add-beds-title')).not.toBeInTheDocument();
      await expect(canvas.queryByTestId('next-assign-beds')).not.toBeInTheDocument();
    ...
    await waitFor(async () => {
      const AddBedsTitle = canvas.getByTestId('add-beds-title');
      const NextButton = canvas.getByTestId('next-assign-beds');
      await expect(canvas.queryByTestId('assign-beds-title')).not.toBeInTheDocument();
      await expect(canvas.queryByTestId('back-add-beds')).not.toBeInTheDocument();
      await expect(AddBedsTitle).toBeInTheDocument();
      await expect(NextButton).toBeInTheDocument();
    });
  },
