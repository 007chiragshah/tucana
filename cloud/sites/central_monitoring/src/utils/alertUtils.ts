...
import { BATTERY_STATUS_VALUES } from './sensorUtils';
 ...
 */
export const getHighestPriorityWaveformAlert = (
  alerts: DeviceAlert[],
  ...
  if (
    sensorMetrics &&
    sensorMetrics[WS_CODES.DEVICE_BATTERY_STATUS] &&
    sensorMetrics[WS_CODES.DEVICE_BATTERY_STATUS].value === BATTERY_STATUS_VALUES.CHARGING
  ) {
    return {
      type: ALERT_TYPES.DEVICE,
      id: 'charging',
      code: DEVICE_ALERT_CODES.SENSOR_CHARGING_ALERT.code,
      deviceCode: '',
      priority: ALERT_PRIORITY.INTERNAL,
      acknowledged: false,
      timestamp: '',
      waveformMessage: DEVICE_ALERT_CODES.SENSOR_CHARGING_ALERT.waveformMessage,
    };
  }
...
const SessionAlertMessageByCodeAndSensor: Record<string, Record<string, string>> = {
  ...
  '258106': {
    ...
    'Viatom BP monitor': 'BP2A - critical battery (< 9%)',
  },
  '258102': {
    ...
    'Viatom BP monitor': 'BP2A - low battery (< 20%)',
    'Nonin 3150': 'Nonin 3150 - low battery (< 20%)',
  },
  '258138': {
