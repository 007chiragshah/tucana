...
import { BATTERY_STATUS_VALUES, checkMetricSensorCharging } from '@/utils/sensorUtils';
...
const VitalsTab = ({
  ...
  const anneChestSensor = useMemo(
    () => activeSensors.filter((sensor) => sensor.type === DEVICE_CODES.ANNE_CHEST.type)[0],
    [activeSensors]
  );
  const noninSensor = useMemo(
    () => activeSensors.filter((sensor) => sensor.type === DEVICE_CODES.NONIN_3150.type)[0],
    [activeSensors]
  );
  const anneLimbSensor = useMemo(
    () => activeSensors.filter((sensor) => sensor.type === DEVICE_CODES.ANNE_LIMB.type)[0],
    [activeSensors]
  );
  ...
  const {
    ...
    const anneChestDeviceAlert = getHighestPriorityWaveformAlert(
      ...
      sensorMetrics[anneChestSensor?.primaryIdentifier]
    );
    const noninDeviceAlert = getHighestPriorityWaveformAlert(
      ...
      sensorMetrics[noninSensor?.primaryIdentifier]
    );
    const anneLimbDeviceAlert = getHighestPriorityWaveformAlert(
      ...
      sensorMetrics[anneLimbSensor?.primaryIdentifier]
    );
      ...
      <Grid display='flex' flexDirection='row' minHeight={236}>
        ...
        <HeartRate
          ...
          isError={
            !hasActivePatientSession ||
            technicalAlerts.HR ||
            checkMetricSensorCharging(activeSensors, sensorMetrics, METRIC_INTERNAL_CODES.HR) ||
        ...
        <Grid flex={1} flexDirection='column' className='metricCard' gap={8}>
          <Oximeter
            ...
            isError={
              !hasActivePatientSession ||
              technicalAlerts.SPO ||
              checkMetricSensorCharging(activeSensors, sensorMetrics, METRIC_INTERNAL_CODES.SPO2) ||
          ...
          <RespiratoryPulseRate
            ...
            isError={
              !hasActivePatientSession ||
              technicalAlerts.PR ||
              checkMetricSensorCharging(activeSensors, sensorMetrics, METRIC_INTERNAL_CODES.PR) ||
