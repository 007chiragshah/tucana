...
import { Alert, BedsideAudioAlarmStatus, DeviceAlert, VitalsAlert } from '@/types/alerts';
...
const TruncatedBedIdTitle = styled(Typography)(() => ({
  fontWeight: 500,
  maxWidth: '272px',
  overflow: 'hidden',
      ...
      <Grid display='flex' flexDirection='row'>
        ...
        >
          <Grid display='flex' flexDirection='row' alignItems='center' gap='8px'>
            {bedsideStatus[associatedMonitor] === BedsideAudioAlarmStatus.PAUSED ||
            bedsideStatus[associatedMonitor] === BedsideAudioAlarmStatus.OFF ||
            deviceAlerts.length > 0 ||
            audioAlerts.length > 0 ? (
              <TruncatedBedIdTitle variant='h2'>{`Bed ID ${
                selectedBed.bedNo || ''
              }`}</TruncatedBedIdTitle>
            ) : (
