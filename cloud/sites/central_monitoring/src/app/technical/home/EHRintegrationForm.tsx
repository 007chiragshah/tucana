    ...
    await validate(
      { password },
      {
        ...
        onError: (err: ServerErrorResponse) => {
          setErrors({ password: t(getAuthErrorMessageKey(err)) });
        },
        onSettled: () => {
          setSubmitting(false);
        },
    ...
    <>
      <PasswordValidationModal
        ...
        description={
          ...
          >
            {t('TechnicalHome.passwordValidationDescription')}
          </Typography>
        }
      />
      <IntegrationStatus
      ...
      >
        <Box>
          <Typography variant='h1'>{t('TechnicalHome.title')}</Typography>
          <Box height={8} />
          <Typography fontFamily={openSansFont.style.fontFamily} variant='subtitle1'>
            {t('TechnicalHome.subtitle')}
          </Typography>
        </Box>
        <form>
          <LabelContainer htmlFor='host'>
            <LabelText>{t('TechnicalHome.hostAddressTitle')} </LabelText>
            <LabelHelper>({t('TechnicalHome.required')})</LabelHelper>
          </LabelContainer>
          <StyledTextField
            fullWidth
          ...
          <LabelContainer htmlFor='port'>
            <LabelText>{t('TechnicalHome.serverPortTitle')} </LabelText>
            <LabelHelper>({t('TechnicalHome.required')})</LabelHelper>
          </LabelContainer>
          <StyledTextField
            fullWidth
          ...
          <LabelContainer htmlFor='interval'>
            <LabelText>{t('TechnicalHome.transmisionIntervalTitle')} </LabelText>
            <LabelHelper>({t('TechnicalHome.required')})</LabelHelper>
          </LabelContainer>
          <StyledTextField
            fullWidth
          ...
          {formik.touched.interval && formik.errors.interval ? (
            ...
            <ErrorText color={theme.palette.grey[600]} variant='error'>
              {t('TechnicalHome.transmisionIntervalInstructions')}
            </ErrorText>
          )}
          <Box height={24} />
