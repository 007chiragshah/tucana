...
export const StyledBedCheckboxList = styled(Grid)(({ theme }) => ({
  overflowY: 'scroll',
  overflowX: 'hidden',
  '::-webkit-scrollbar': {
    width: '4px',
  ...
  return (
    <>
      ...
      >
        ...
        <Content style={{ width: 'calc(33% - 8px)', padding: 0 }}>
          <ContentHeader sx={{ padding: '6px 16px 0 16px' }}>
            <Grid display='flex' sx={{ alignItems: 'center' }}>
              <BedIcon />
              <Typography variant='h2'>
          ...
          {beds.isError ? (
            <Typography
              sx={{ margin: (theme) => theme.spacing(10, 0, 8, 0), padding: '0 16px 0 16px' }}
              variant='error'
            >
              {bedManagementErrors.failedToFetch}
            </Typography>
          ) : (
            <StyledBedCheckboxList sx={{ p: '0 0 4px 16px', mb: '2px', mr: '2px' }}>
              {Object.keys(tempGroupsBeds).length > 0 ? (
