...
const AddBedsContent = ({
  ...
  const getBedError = (bed: BedType) => {
    if (errors[bed.id]) return errors[bed.id];
    else if (bed.bedNo.length >= maxBedIdLength) {
      return bedManagementErrors.maximumBedIdLengh;
    }
                  ...
                  {beds.map((bed: BedType, index: number) => (
                    ...
                    >
                      ...
                      <Grid item display='flex' flexDirection='column'>
                        <BedTextField
                          error={!!errors[bed.id] || bed.bedNo.length >= maxBedIdLength}
                          sx={{
                            '& fieldset': {
                              borderColor:
                                errors[bed.id] || bed.bedNo.length >= maxBedIdLength
