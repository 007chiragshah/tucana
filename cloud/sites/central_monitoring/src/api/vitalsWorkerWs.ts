...
import { RawSocketMessage, RawSocketMessagePayload } from '@/types/socketMessage';
...
function sendMetrics(payload: RawSocketMessagePayload | undefined, pid: string | undefined) {
      ...
      case WORKER_FLOWS.CONNECT:
        if (
          ...
          socket.onmessage = (message) => {
            ...
            } else if (eventType === EVENT_CODES.DISCONNECT) {
              postMessage({
                status: RESPONSE_STATUS.DEVICE_REFRESH,
                deviceId: messageWithMetadata.pm_id || '',
                isPatientMonitor: false,
              });
            ...
            } else if (eventType === EVENT_CODES.NEW_METRICS) {
              sendMetrics(payload, pid);
            } else if (eventType === EVENT_CODES.DEVICE_DISCOVERED) {
              const initialMetricValues: RawSocketMessagePayload[] =
                payload?.initial_metric_values || [];
              for (const metric of initialMetricValues) {
                sendMetrics(metric, metric.patient_primary_identifier);
              }
            }
          };
