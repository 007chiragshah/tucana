...
paths=(
    ...
    "./platforms/auth/scripts/create_env.sh"
...
migrations_commands=(
    ...
    "docker exec -it auth python -m alembic upgrade head"
...
output=$(docker exec -it auth python create_user.py --username admin@sibelhealth.com --roles clinical --password pass1234 2>&1)
echo -e "${GREEN}$output ${NC}"
...
echo -e "${GREEN}$output ${NC}"
output=$(docker exec -it auth python create_user.py --username tech@sibelhealth.com --roles tech --password pass1234 2>&1)
echo -e "${GREEN}$output ${NC}"
