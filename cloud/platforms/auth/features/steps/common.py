...
@step("the requester user exists")
def step_impl(context):
    user = User(
        id=context.sub,
        username=context.username,
        password=crypto.hash_password(context.password),
    )
    context.db.add(user)
    context.db.flush()

    stmt = select(Role).where(Role.name.in_([r.value for r in context.roles]))
    found_roles = context.db.execute(stmt).scalars().all()
