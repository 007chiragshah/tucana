...
def get_installer_images() -> list[Image]:
    ...
    return [
        ...
        if img.tags and installer_regex.match(img.tags[0])
    ]


def get_installer_images_names() -> list[str]:
    images = get_installer_images()
    return [
        img.tags[0].split(":")[1]
        for img in images
        if img.tags and len(img.tags[0].split(":")) >= 2
    ]
