...
def get_devices_list_filtered_by_bed(context):
    url = "/web/device?bed_id=" + context.bed_id
    context.response = requests_get(context, url)
...
def verify_device_information(context):
    ...
    if not device_list_filtered.get("resources"):  # Using `.get()` to avoid KeyError
        raise AssertionError("No devices found in the response.")

    # Flag to indicate if a matching device has been found
    device_found = False

    # Iterate through each device in the resources list
    for device in device_list_filtered["resources"]:
        # Check that the required fields exist in the device data
        if not all(key in device for key in ["name", "primary_identifier", "gateway_id"]):
            raise AssertionError("One or more required keys are missing from the device data.")
        
        # Check if the device matches the expected values
        if (
            device["name"] == context.device_name and
            device["primary_identifier"] == context.device_identifier
            # device["gateway_id"] == context.gateway_id
        ):
            # If a matching device is found, set the flag to True
            device_found = True
            break  # Exit the loop since we found the matching device

    # Raise an error if no matching device was found after iterating through all resources
    if not device_found:
        raise AssertionError(
            f"No device found with the specified parameters: "
            f"Name: {context.device_name}, "
            f"Identifier: {context.device_identifier} "
            # f"Gateway ID: {context.gateway_id}."
        )
...
def bed_device_exist(context):
    ...
    elif context.env_config["environment"].upper() == "DEV":
        context.bed_id = "b7aee215-a6dc-4a1c-b4b7-e742e6ed67ec"
        context.device_uuid = "00000000-0000-0000-0000-000000000001"
    elif context.env_config["environment"].upper() == "LOCAL":
        context.bed_id = "b7aee215-a6dc-4a1c-b4b7-e742e6ed67ec"
        context.device_uuid = "00000000-0000-0000-0000-000000000001"
