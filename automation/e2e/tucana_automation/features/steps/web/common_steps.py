...
@step("Tom sees a message stating that the account is locked due to too many failed attempts")
def incorrect_password_failed_attempts(context):
    context.current_page.too_many_attempts_error()
...
@step("Tom sees a message stating that the account is locked due to too many failed attempts on change password")
def incorrect_password_failed_attempts(context):
    assert context.current_page.check_password_message(
        "Too many attempts. Please try again in 15 minutes."
    ), "Expected message 'Too many attempts. Please try again in 15 minutes. wasn't present"
...
@step("Tom should see the password placeholder")
def check_password_placeholder_presence(context):
    context.current_page.check_password_placeholder()

@step("Tom sees the placeholder for the new password")
def check_new_password_field_placeholder_presence(context):
    context.current_page.new_password_field_placeholder_presence()

@step("Tom sees the placeholder for the re-enter new password")
def check_re_enter_new_password_field_placeholder_presence(context):
    context.current_page.re_enter_new_password_field_placeholder_presence()

@step("the application notifies him Password changed successfully")
def check_password_changed(context):
    context.current_page.password_changed_presence()

@step("the application notifies him Please use your new password for future logins")
def check_use_your_new_password_for_future_logins(context):
    context.current_page.use_your_new_password_for_future_logins_presence()

@step("Tom clicks on the ok button")
def clicks_ok_button(context):
    context.current_page.click_on_ok_button()


@step("Tom wants to dismiss th button")
def clicks_dismiss_button(context):
    context.current_page.click_on_dismiss_button()
