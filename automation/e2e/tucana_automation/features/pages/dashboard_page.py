...
class DashboardPage(BasePage):
    anne_logo = (By.CSS_SELECTOR, "svg[viewBox='0 0 82 42']")
    ...
    dismiss_btn = (By.XPATH, "//div[@data-testid='dismiss-modal']")
    log_out_error_msg = (
        By.XPATH,
        "//span[text()='Incorrect password. Please try again.']",
    )
    too_many_attempts_error_msg = (
        By.XPATH,
        "//span[text()='Too many attempts. Please try again in 15 minutes.']",
    )
    ...
    password_error_message = (By.XPATH, "//span[text()='{}']")
    ...
    new_password_field_placeholder = (By.XPATH, "//input[@placeholder='New password']")
    re_enter_new_password_field_placeholder = (By.XPATH, "//input[@placeholder='Re-enter new password']")
    password_changed = (By.XPATH, "//h1[text()='Password changed']")
    use_your_new_password_for_future_logins = (By.XPATH, "//h6[text()='Your password has been successfully changed. Please use your new password for future logins.']")
    ...
    def click_on_dismiss_button(self):
        try:
            self.web_utils.find_element(*self.dismiss_btn).click()
        except Exception:
            raise Exception("Can not find the dismiss button")
        
    def click_on_ok_button(self):
        try:
            self.web_utils.find_element(*self.ok_button).click()
        except Exception:
            raise Exception("Can not find the ok button")
        
    def new_password_field_placeholder_presence(self):
        try:
            new_password_field_placeholder = self.web_utils.find_element(*self.new_password_field_placeholder)
            return True if new_password_field_placeholder.is_displayed() else False
        except Exception:
            raise Exception("Can not find the new password placeholder")
        
    def re_enter_new_password_field_placeholder_presence(self):
        try:
            re_enter_new_password_field_placeholder = self.web_utils.find_element(*self.re_enter_new_password_field_placeholder)
            return True if re_enter_new_password_field_placeholder.is_displayed() else False
        except Exception:
            raise Exception("Can not find the re enter new password placeholder")
        
    def password_changed_presence(self):
        try:
            Password_changed = self.web_utils.find_element(*self.password_changed)
            return True if Password_changed.is_displayed() else False
        except Exception:
            raise Exception("Can not find the Password changed message ")
        
    def use_your_new_password_for_future_logins_presence(self):
        try:
            use_your_new_password_for_future_logins = self.web_utils.find_element(*self.use_your_new_password_for_future_logins)
            return True if use_your_new_password_for_future_logins.is_displayed() else False
        except Exception:
            raise Exception("Can not find the use your new password for future logins message ")
